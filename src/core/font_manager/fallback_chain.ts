 ...
 */
class DefaultDictFallbackHandler implements FontFallbackHandler {
  readonly name = "default-dict";
 ...
 */
class TerminalErrorFontHandler implements FontFallbackHandler {
  readonly name = "terminal-error-font";

  handle(context: FontFallbackContext): FontFallbackResult {
    return {
      strategy: this.name,
...
export class FontFallbackChain {
  /** 处理器列表，按优先级从高到低排列。 */
  readonly #handlers: FontFallbackHandler[];
  ...
  removeHandler(name: string): boolean {
    const index = this.#handlers.findIndex(
      (handler, position) =>
        handler.name === name && position < this.#handlers.length - 1
    );
    if (index < 0) {
      return false;
    }
