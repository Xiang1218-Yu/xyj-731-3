...
export class FontManager {
  ...
  /** 内置 CMap 字节缓存。 */
  readonly #cmapCache: LruCache<string, BuiltInCMapData>;

  /** 标准字体原始字节缓存。 */
  readonly #standardFontCache: LruCache<string, Uint8Array>;

  /** 内置 CMap 异步加载器。 */
  readonly #cmapLoader: AsyncCMapLoader;

  /** 标准字体加载的 in-flight 表（并发去重，语义同 CMap 加载器）。 */
  readonly #standardFontInFlight = new Map<
    string,
    Promise<Uint8Array | null>
  >();

  /** 智能字体回退链。 */
  readonly #fallbackChain: FontFallbackChain;

  /** 私有构造：禁止 `new`，实例只能经 `getInstance()` 获取。 */
  private constructor() {
    this.events = new FontEventBus();
    this.#cmapCache = new LruCache<string, BuiltInCMapData>({
      onEvict: key => this.#emitEvict("builtin-cmap", key),
    });
    this.#standardFontCache = new LruCache<string, Uint8Array>({
   ...
   */
  async loadStandardFontData(
    name: string,
    fetcher: StandardFontDataFetcher
  ): Promise<Uint8Array | null> {
  ...
  /** 广播缓存逐出事件（LRU 回调）。 */
  #emitEvict(namespace: CacheNamespace, key: string): void {
